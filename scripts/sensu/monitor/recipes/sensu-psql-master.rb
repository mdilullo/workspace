...
sensu_check "db-#{psql_config['database']}-graphite" do
    command "metric-postgres-graphite.rb -m #{node.ipaddress} -s #{node.ipaddress} -u #{psql_config['username']} -d #{psql_config['database']} -p #{psql_config['password']}"
    handlers ["graphite"]
    type "metric"
    standalone true
    interval 30
...
sensu_check "db-#{psql_config['database']}-replication" do
    command "check-postgres-replication.rb -m #{node.ipaddress} -s #{node.ipaddress} -u #{psql_config['username']} -d #{psql_config['database']} -p #{psql_config['password']}"
    handlers [handler]
    standalone true
    interval 30
    timeout 10
